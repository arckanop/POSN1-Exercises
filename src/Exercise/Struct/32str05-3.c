...
int main() {
	...
	printf("Enter the student id of the student that you want the data of: ");
	scanf("%d", &targetID);

	for (i = 0; i < 5; i++) {
		if (targetID == record[i].id) {
	...
	if (found == 0) printf("Student with the ID %d not found!", targetID);
